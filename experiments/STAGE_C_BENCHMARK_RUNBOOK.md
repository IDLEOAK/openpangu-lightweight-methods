# Stage C Benchmark Runbook

## Goal

Stage C evaluates the final Stage B compressed artifacts on the narrowed benchmark line:

1. `MMLU` 4-task English subset
2. `C-Eval` 4-task Chinese subset

The baseline uses the original dense model.
All compressed methods use their Stage B `compressed_artifact/` outputs.

## Task Set

The current Stage C task group is `stage_c_core`, containing 8 tasks in a balanced `4 + 4` layout:
...
1. This Stage C line is intentionally narrower than the earlier 15-task transition benchmark, but more balanced than the old `6 + 2` split.
2. Stage C evaluates the final Stage B compressed artifacts, not the old in-memory-only method states.
