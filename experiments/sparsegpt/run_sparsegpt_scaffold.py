...
def evaluate_benchmark_single_device(model, tokenizer, benchmark_plan, system_prompt, runtime_device):
    if runtime_device.type != "cuda":
        return {"skipped": True, "reason": "benchmark evaluation requires cuda runtime"}
...
def main() -> int:
    ...
    if sparse_mode == "scaffold":
        ...
        compressed_artifact_dir = resolve_path(run_dir, config["sparsegpt"].get("compressed_artifact_dir"))
