...
from experiments.smoothquant.algorithm import (
    build_calibration_batch,
    evaluate_openpangu_perplexity_sequential,
    quantize_openpangu_smoothquant_sequential,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SmoothQuant scaffold for OpenPangu.")
    ...
    parser.add_argument("--save-dir", default="", help="Optional override for config.smoothquant.save_dir.")
    parser.add_argument("--experiment-name-suffix", default="", help="Optional suffix appended to experiment_name.")
    parser.add_argument("--min-layer", type=int, default=None, help="Optional override for smoothquant.min_layer.")
    parser.add_argument("--max-layer", type=int, default=None, help="Optional override for smoothquant.max_layer.")
    parser.add_argument("--bits", type=int, default=None, help="Optional override for smoothquant.bits.")
    parser.add_argument("--group-size", type=int, default=None, help="Optional override for smoothquant.group_size.")
    ...
    if args.save_dir:
        config["smoothquant"]["save_dir"] = args.save_dir
    ...
    if args.min_layer is not None:
        config["smoothquant"]["min_layer"] = args.min_layer
    if args.max_layer is not None:
        config["smoothquant"]["max_layer"] = args.max_layer
    if args.bits is not None:
        config["smoothquant"]["bits"] = args.bits
    if args.group_size is not None:
        config["smoothquant"]["group_size"] = args.group_size
    ...
    run_dir = create_run_dir(output_root, "smoothquant", config["experiment_name"])
    ...
    summary = {
        ...
        "smoothquant_plan": {
            "bits": config["smoothquant"]["bits"],
            "group_size": config["smoothquant"]["group_size"],
            "alpha": config["smoothquant"]["alpha"],
        },
        "source_validation": {
            "paper_reference": "Xiao et al. 2023 SmoothQuant",
            "expected_upstream_snapshot": str((REPO_ROOT.parent / "external" / "smoothquant").resolve()),
            "scaffold_reuse": [
    ...
    quant_stats = quantize_openpangu_smoothquant_sequential(
        model,
    ...
    save_dir = resolve_path(run_dir, config["smoothquant"].get("save_dir"))
    ...
    compressed_artifact_dir = resolve_path(run_dir, config["smoothquant"].get("compressed_artifact_dir"))
    ...
    write_json(run_dir / "smoothquant_stats.json", quant_stats)
    ...
    print("[OK] SmoothQuant sequential quantization port executed.")
    return 0
